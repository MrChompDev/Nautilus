...
PROJECT = {
    ...
    "aesthetic": "glassmorphism: semi-transparent glass surfaces (glass_bg/glass_bg_dark/glass_edge), rounded corners (12px), edge highlights, no backdrop-filter (Qt6 QSS limitation). seafoam #00F2C2 on abyss navy #081626",
    "license": "MIT",
...
APPS = {
    ...
    "kraken": {
        "desc": "Local-first agentic AI engine + multi-agent workforce with 4 sea-creature themed models. Two surfaces: CLI (kraken.py -> cli.py) and PySide6 ChatGPT-style desktop (main.py -> ui/). Engine is pure stdlib, events marshalled via polled queue to Qt thread. Models: Kraken (coding), Leviathan (writing), Charybdis (image/video gen), Megalodon (pentest). Each has custom SVG logo + dedicated engine. All 4 models trained from scratch with NumPy GPT + BPE tokenizer.",
        "ram": 120,
        "structure": {
            "core/config.py": "CREATURES dict (kraken/leviathan/charybdis/megalodon with name/color/subtitle/description), KrakenConfig dataclass, JSON at ~/.kraken/config.json",
            "core/models.py": "ModelRegistry (lazy-loads engine instances per creature), ModelHandle (wraps respond() calls)",
            "core/engine.py": "BaseEngine abstract class, EngineResponse dataclass",
            "core/tools.py": "Shared tool system: file_read/write/list, terminal_exec (sandboxed to workspace)",
            "core/memory.py": "MemoryStore SQLite (~/.kraken/memory.db): remember/recall with cosine similarity",
            "models/kraken/engine.py": "KrakenEngine: coding specialist, project brain context, code block extraction, file ops via tools",
            "models/leviathan/engine.py": "LeviathanEngine: prose writing specialist, formatting-aware, creative rewrite/proofread/brainstorm",
            "models/charybdis/engine.py": "CharybdisEngine: procedural image gen (plasma, fractals, ocean waves, pixel creatures), PNG export to output/",
            "models/megalodon/engine.py": "MegalodonEngine: security auditor, 12 vulnerability patterns, port scanner, dependency checker, full audit reports",
            "engine/spec.py": "Markdown Agent Builder: frontmatter -> AgentSpec. KNOWN_TOOL_NAMES, KNOWN_ROLES. SpecError.",
            "engine/agent_store.py": "AgentStore catalog of .md agents in ~/.kraken/agents/; CRUD, import, resolve, role_spec(role).",
            "engine/providers.py": "ChatClient streaming over HTTP (OpenAI SSE, Ollama native, Anthropic native). ProviderError.",
            "engine/local.py": "nautilus provider bridge: loads models/lm engine + brain context; model fallback kraken->leviathan->megalodon",
            "engine/brain.py": "Project brain: scan files -> SQLite; context() top-k keyword overlap; CLI: brain scan|status|context",
            "engine/keys.py": "Key resolution: ~/.kraken/keys.json > ~/.env > env vars. save_keys 0600, never logged.",
            "engine/memory.py": "MemoryStore SQLite with token-based pseudo-embedding cosine recall",
            "engine/agent.py": "Agent single-loop w/ Self-Correction Loop. parse_tool_blocks regex. max_rounds=12.",
            "engine/orchestrator.py": "Workforce: Planner -> parallel exec -> QA/Review -> report. max_parallel=3",
            "engine/tools.py": "Tool registry: file_read/write/delete/list, terminal_exec (300s timeout). PermissionGate FAIL-CLOSED.",
            "cli.py": "REPL + subcommands (build, doctor, models, memory, config, agent, brain, keys, setup)",
            "ui/main_window.py": "KrakenWindow: ChatGPT/Claude-style layout with creature selector sidebar, conversation history, settings dialog, export, keyboard shortcuts (Ctrl+1-4 switch models, Ctrl+N new chat, Ctrl+E export)",
            "ui/chat_panel.py": "ChatPanel: message bubbles (user right, assistant left), typing indicator, streaming, welcome screen with quick actions",
            "ui/creature_selector.py": "CreatureCard-based model picker: colored logo circles, active state highlighting",
            "ui/logo_loader.py": "SVG→QPixmap loader using QSvgRenderer from PySide6.QtSvg",
            "logos/*.svg": "Custom SVG logos: kraken (teal octopus), leviathan (purple serpent), charybdis (pink whirlpool), megalodon (red shark)",
        },
        "models_trained": "models/trained/{kraken,leviathan,charybdis,megalodon}/ each: weights.npz (~295KB int8), bpe.json (1024 vocab), model.json, config.json. Trained with models/lm/train.py (tiny profile, 80 steps, CPU)",
        "persistence": "~/.kraken/: config.json (0600), memory.db (0600), keys.json (0600), agents/*.md. Engine is pure stdlib.",
        "packaging": "pyproject.toml: kraken-ai 1.0.0; console scripts kraken=apps.kraken.cli:main, kraken-gui=apps.kraken.main:main; [gui] extra adds PySide6",
    },
}
