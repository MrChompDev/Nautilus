...
class LogAnalyzerWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        layout.addWidget(make_header(
            ...
            "Read the wake - detect anomalies in system logs"
        ))
        layout.addWidget(hline())

        row = QHBoxLayout()
        self.load_btn = QPushButton("Load sample log")
        ...
        self.log_input.setPlaceholderText("Paste log contents here, or load the sample...")
    ...
    def _analyze(self):
        ...
        # Detect brute force (4+ failures from same IP)
        ...
        if suspicious_ip:
            self.output.setTextColor(RED)
            self._log(f"\n[!] RECOMMENDATION: Block {suspicious_ip} and enable fail2ban.")
        if wget_urls:
            self.output.setTextColor(RED)
            self._log(f"\n[!] SUSPICIOUS DOWNLOAD from privileged user: {wget_urls[0]}")
        if not suspicious_ip and not wget_urls:
            self.output.setTextColor(GREEN)
